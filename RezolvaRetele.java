...
public class RezolvaRetele extends Task {
    public int n; // members / nodes
    public int m; // friendships / edges
    public int k; // the dimension of the group / of the clique
    public int[][] matrix; // the friendships between members
    public int[][] adiacent_matrix;

    public RezolvaRetele() { }
    ...
    public void formulateOracleQuestion() throws IOException {
        ...
        for (int u = 1; u <= n; u++) {
            for (int v = 1; v <= n; v++) {
                if (getAdiacent_matrix()[u][v] == 0 && u != v) {
                    for (int i = 1; i <= k - 1; i++) {
                        for (int j = i + 1; j <= k; j++) {
                            int x = (i - 1) * n;
                            x += u;
